...
| 5 | Remove "the Home Office calls the later figure **the latest recorded outcome**". The Home Office does not use that phrase | grant-rate record notes, and `content/claims/refused-asylum-seekers-are-eventually-recognised.md:54` | Session |
| 6 | Correct **"between 2010 and 2020"** wherever the publisher's 17-to-29 range is quoted, or stop attributing that range to a narrowed window. **Three sites, not one**: the record notes, `content/asylum.njk:95` and `content/claims/refused-asylum-seekers-are-eventually-recognised.md:44`. Also the invented compound "latest recorded grant rate" at `content/asylum.njk:96` and the same claim page | grant-rate record notes, `asylum.njk`, `content/claims/refused-asylum-seekers-are-eventually-recognised.md` | Session |
...
| 18e | Reword the refused-asylum short answer from "ended without a grant" to "had not ended in a grant at the latest recorded outcome". K2 | `content/claims/refused-asylum-seekers-are-eventually-recognised.md:5` | Owner |
...
| 18i | **DONE.** The CI step name no longer carries a count: it reads "on every URL in .pa11yci.json", so it cannot go stale when a page is added. I1b | `.github/workflows/validate-data.yml` | Session |
