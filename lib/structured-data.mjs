...
//
// No figure and no publisher's words reach this file. The name, strapline and URL come from
// content/_data/site.js, the description from data/meta.json, the file list and the modification
// date from the data layer itself, so none of those is a second home.
//
// Four values ARE written here, and the first claim of this paragraph said none were until a
// second model read it: `en-GB`, `United Kingdom`, `application/json` and the schema.org type
// names. Three are properties of the vocabulary rather than facts about this project. `en-GB` is
// the exception and is a genuine second home, the first being `<html lang="en-GB">` in
// content/_includes/base.njk, which scripts/check-build.mjs asserts by that exact string. Moving
// it into site.js would touch the template and that check for a value that has never changed, so
