...
// above it does not come from. The charts' visible source lines ARE typed, and since 19 August
// 2026 `checkSourceLine` in lib/charts.mjs refuses one naming a publication the chart's own points
// do not cite: a citation typed a second time would be a second home for a fact the data layer
// already holds, which is the defect this project has closed everywhere else.
//
...
// professional following it in a browser gets a file rather than a page. Many source_urls in
// the data layer are one of these.
//
...
// review's reasoning was that these URLs "sit on series points that emit no link", and some of
// them sit on metric records rather than on points. Corrected here rather than carried, because
...
export function citationBlock({ sources, url, label }) {
  ...
  const lines = [...cited.values()].map((source) => {
    ...
    return `
      <li>${escape(stop(name))}
      ${cited_for}${source.published_date ? `Published <time datetime="${escape(source.published_date)}">${escape(longDate(source.published_date))}</time>. ` : ''}<a href="${escape(source.source_url)}">${escape(source.source_url)}</a>${download ? ' (spreadsheet download)' : ''}.
      Checked by us on <time class="checked" datetime="${escape(source.retrieved_date)}">${escape(longDate(source.retrieved_date))}</time>.</li>`;
  }).join('');
