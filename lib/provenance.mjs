...
// block's job, it is under every chart already, and whether it should name the figure as well is
// an open decision on `metric_name` that this must not settle by omission.
...
export function provenanceList({ metrics, pageUrl }) {
  ...
  const rows = metrics.map((metric) => {
    ...
    <div class="provenance-row">
      ...
      <span class="provenance-checked">Checked ${escape(longDate(metric.retrieved_date))}</span></dd>
    </div>`;
  }).join('');
