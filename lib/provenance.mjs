...
// shape here would put every row of provenance into a horizontal scroller on the pages a phone
...
// definitions, which is how the home page introduces the same vocabulary; a link on every
// identical grade word would be one route to one page repeated once per figure.
//
...
export function provenanceList({ metrics, pageUrl }) {
  ...
  const rows = metrics.map((metric) => {
    ...
    <div class="provenance-row">
      ...
      <span class="provenance-checked">Checked by us on <time class="checked" datetime="${escape(metric.retrieved_date)}">${escape(longDate(metric.retrieved_date))}</time></span></dd>
    </div>`;
  }).join('');
