...
// data, `source` and `sourceUrl`; what checks it against the data, and the incident that made
// that necessary, are two paragraphs down.
//
...
export function lineChart({ id, series, title, summary, unit, source, sourceUrl, note, breakAfter, pageUrl }) {
  ...
  // After the guards above, so an empty or one-year series is named as that rather than as a
  // chart with no source.
  checkSourceLine(id, sourceUrl, points);
