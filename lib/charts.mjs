...
const GEOM = {
  wide: { key: 'wide', W: 760, H: 340, pad: PAD, yInside: false, legend: false, xTarget: 8, barRow: 52, barGutter: 210 },
  ...
  narrow: { key: 'narrow', W: 360, H: 300, pad: { top: 64, right: 10, bottom: 40, left: 2 }, yInside: true, legend: true, xTarget: 4, barRow: 66, barGutter: 0 },
};
...
function linePlot({ series, unit, breakAfter }, g) {
  ...
  const yTicks = yValues.map((v) => `
    ...
    <text class="axis" x="${g.yInside ? leftPad + 2 : leftPad - 10}" y="${y(v) + (g.yInside ? -5 : 4)}" text-anchor="${g.yInside ? 'start' : 'end'}">${format(v)}</text>`).join('');
  ...
  // The end labels are anchored to the plot edge rather than centred on their tick. Centred, the
  // first and last lose half their width off the SVG: with the narrow chart's 2px left padding
  // "2012" rendered as "012" and "2025" as "202", which is a wrong year rather than a clipped one.
  ...
  const xTicks = years.filter((_, i) => i === last || (i % step === 0 && last - i >= step)).map((year) => {
    ...
    const anchor = first ? 'start' : final ? 'end' : 'middle';
    return `
  ...
  const legend = g.legend ? series.map((s, i) => {
    ...
    // on "1,500,000".
    return `
    <line class="series series-${i}${s.dashed ? ' dashed' : ''}" x1="${swatchX}" y1="${LEGEND_Y}" x2="${swatchX + 22}" y2="${LEGEND_Y}"></line>
