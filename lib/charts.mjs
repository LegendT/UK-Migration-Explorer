...
// The CITATION BLOCK is built from the points the chart draws, not from an argument beside them,
// so it cannot cite a publication its own data does not come from. The `points` filter carries
// each point's source fields through for this; the plot itself reads only year and value.
// THAT GUARANTEE STOPS AT THE CITATION BLOCK, and this comment claimed it for the whole chart
// until 11 August 2026. The visible `Source:` line below takes exactly the argument beside the
// data, `source` and `sourceUrl`, and nothing compares the two: on /migration/ both line charts
// linked a page their plotted series is not in, one of them containing no figure at all, while
// the citation block under the same chart named the spreadsheet. Deriving this line from the
// points as well is The order's item 41, and it has to decide what one line says when a chart's
// points disagree, so it is not done here.
//
// IT SAID "the inversion The order's item 27 describes" UNTIL 19 AUGUST 2026, AND THAT POINTED AT
// A REMEDY NOBODY WAS GOING TO BUILD. Item 27's inversion was refuted on 12 August by reproducing
// its premise, item 27 closed on a different one, and this comment and one sentence in item 17
// were the only two things aiming this work anywhere. Found by running the both-directions grep
// that closing an item requires, which is what item 41 exists as the result of.
// THE `Source:` LINE IS NOW COMPARED WITH THE CHART'S OWN DATA, which is what nothing did until
// 19 August 2026 and is The order's item 41. The line takes the `source` and `sourceUrl` written
// beside the chart; the citation block under it is built from the points or records the chart
// actually draws. On 11 August 2026 both line charts on /migration/ linked a page their plotted
// series is not in, one of them carrying no figure at all, while the citation block named the
// spreadsheet. Every chart was corrected by hand and nothing stopped it happening again.
//
// IT CHECKS RATHER THAN DERIVES, and that is the owner's decision of 19 August 2026 rather than a
// shortcut. A chart may draw from more than one publication and one line can name one of them:
// `initial-decisions-by-outcome` on /asylum/ takes three bars from two publications, and the
// decision is that its line names the summary tables, which two of the three are in and which its
// own label already said, while the citation block names both. So the rule is that the line must
// name a publication the chart's data actually cites, not that it must name all of them. Deriving
// the line instead would have to invent an answer for the case this rule permits.
function checkSourceLine(id, sourceUrl, cited) {
  const urls = new Set(cited.map((item) => item?.source_url).filter(Boolean));
  if (!urls.size) {
    throw new Error(`Chart "${id}" has no source_url on any of the points or records it draws, so its "Source:" line cannot be compared with anything.`);
  }
  if (!urls.has(sourceUrl)) {
    throw new Error(`Chart "${id}" links ${sourceUrl} in its "Source:" line, and none of the ${urls.size} publication(s) its own data cites is that: ${[...urls].join(', ')}. The line takes the argument written beside the chart and the citation block is built from the data, so a reader clicking "Source:" would leave for somewhere the plotted figures are not. Point it at one of those, or correct the data.`);
  }
}

export function lineChart({ id, series, title, summary, unit, source, sourceUrl, note, breakAfter, pageUrl }) {
  checkSourceLine(id, sourceUrl, series.flatMap((s) => s.points));
...
function barPlot({ bars }, g) {
  ...
  const rows = bars.map((bar, i) => {
    ...
    <text class="bar-value${inside ? ' inside' : ''}" data-metric="${escape(bar.ref)}" x="${(labelW + width + (inside ? -10 : 10)).toFixed(1)}" y="${barTop + 18}"${inside ? ' text-anchor="end"' : ''}>${format(bar.value)}</text>`;
  }).join('');
...
export function barChart({ id, bars, title, summary, unit, source, sourceUrl, note, pageUrl }) {
  checkSourceLine(id, sourceUrl, bars.map((b) => b.record));
