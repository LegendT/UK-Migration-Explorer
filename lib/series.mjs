  ...
  for (const [name, file] of Object.entries(series)) {
    ...
    for (const [block, data, unit] of blocks) {
      for (const point of data.data ?? []) {
        ...
        // `confidence_level` is carried because the `series_ref` guard compares it. It was
        // dropped here, so a metric and the point it names could disagree about the grade with
        // nothing able to ask: the check was keyed on the one field that happened to agree, which
        // is the shape that makes a check permanently satisfied.
        points.set(ref, { block, year, value: point.value, unit, confidence_level: point.confidence_level });
      }
    }
  }
  return points;
