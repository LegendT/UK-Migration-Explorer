...
content/                Eleventy input
  ...
  _data/site.js           Name, strapline, navigation, rendered as a disclosure below 40em
...
`<time class="checked">`, which the three trace emitters write and a `timeTag` date deliberately is
not, and takes the earliest, so a page states when its figures were last
...
The audit is a regression guard, not a proof: pa11y cannot tell a `strong` doing a heading's job
from a heading, or a list of bold labels from the description list it should be. These are checked
by hand when a template changes, and were swept site-wide on 22 September 2026 against the same
checklist ukpublicfinances.org uses.

- A headline figure with an explanation and a source is an `article` with an `h3`, inside its
  list item: the dashboard cards on `/`, whose source line is a `dl` of source, published, checked
  and confidence. A claim check is an `article` with a heading: the claim
  pages, the claims index and the three cards on `/`.
- A label with one value is a `dl` of `dt`/`dd` pairs, label first: the period list on `/`, the
  short answer, the reviewed-period-source row and the misuse and direction pills on a claim page,
  the metadata line on the claims index, the provenance block on every theme page, the confidence levels and the narrow-screen sources list on
  `/sources-and-method/`. Where the list is styled inline, the colon between label and value is
  generated by the stylesheet, so a page without it still lays the pairs out on their own lines.
- A caveat is a `div` with `role="note"` and a real heading, not an `aside`: the correction note on
  a claim page carries an `h2`, so it sits in the outline.
- A `section` with `aria-labelledby` appears nowhere; every page runs bare `h2`s under its `h1`.
- Scrollable table wrappers keep `tabindex="0"` with `role="region"` and a name, because a focusable
  element needs both. `check-build.mjs` refuses one without them, and html-validate's
  `prefer-native-element` rule reports every one of them: that report is declined, not fixed.
- ARIA never repeats a name native HTML already gives. The one `aria-hidden` on a claim card hides
  the visible "The claim" label because the `h1` beside it already carries those words.
- A chart is a `figure` whose `figcaption` holds the `h3` and summary, rendered twice as an SVG with
  `role="img"`, a `title` and a `desc`, the unused rendering hidden with `display: none` so
  assistive technology meets one.
- Abbreviations are expanded in the glossary, not inline; the built site has no `abbr`.
- A date a template prints goes through the `timeTag` filter, so it carries `datetime`; `longDate`
  is for text markup cannot enter. The exception is the three "Checked by us" traces, written by
  hand as `<time class="checked">` in `index.njk`, `lib/citation.mjs` and `lib/provenance.mjs`,
  because the `figure-currency` transform reads only those and a `timeTag` date must never be
  one, or a review, publication or correction date would move a footer's date. A date typed inside markdown prose stays bare,
  because content files are not templates.
- Five of these rules are asked of the built HTML by `check-build.mjs`, probed on every run,
  including that the converted label-value blocks stay `dl`s; the rest, dates, ARIA and sections,
  are a reading when a template changes.
